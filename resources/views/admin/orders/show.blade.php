        ...
        <div class="bg-white rounded-xl shadow-sm border border-[#E8E0D8] p-5 animate-fade-up" style="animation-delay: {{ $cardDelay }}ms">
            ...
            <div class="space-y-2 text-sm">
                <div><span class="text-[#999999]">Nama:</span> <span class="font-medium">{{ $order->customer_name }}</span></div>
                                <div><span class="text-[#999999]">WA:</span> <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $order->customer_whatsapp) }}" target="_blank" rel="noopener noreferrer" class="text-rose-600 hover:underline">{{ $order->customer_whatsapp }}</a></div>
    ...
    <div class="flex flex-wrap gap-3 animate-fade-up" style="animation-delay: {{ $cardDelay + 240 }}ms">
        ...
        @if(in_array($order->status, ['processing', 'ready']))
            <form method="POST" action="{{ route('admin.orders.status', $order->id) }}">
                ...
                <button type="submit"
                        class="px-4 py-2 {{ $order->status === 'processing' ? 'bg-green-500 hover:bg-green-600' : 'bg-purple-500 hover:bg-purple-600' }} text-white text-sm font-medium rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-300">
